...
//close_exec的作用？调用fork函数的时候，子进程会同时复制父进程的文件描述符等，父子进程
//之间共享文件描述符表，但是我们一般会exec函数来运行其他的进程，当然此时保存的文件描述符变量也就不在了，
//所以一般我们就会先close文件描述符，然后再执行exec函数。

//但是当程序十分复杂的时候，子进程甚至不知道fork了多少个文件描述符，此时进行逐一的清理十分困难，
//我们期望在frok某一个子进程前打开某一个文件句柄指定好，“这个文件描述符在我执行fork子进程执行
//exec的时候就关闭。这就是close_on_exec。
...
void sockets::setNonBlockAndCloseOnExec(SOCKET sockfd)
{
    ...
    设置这个之后，fork子进程仍然可以使用这个fd，但是执行exec之后系统就会自动关闭子进程中的fd了。
    // FIXME check
