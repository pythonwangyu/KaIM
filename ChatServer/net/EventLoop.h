...
namespace net
{
    ...
    private:
		...
		const std::thread::id               threadId_;				//loop对应的threadid。
		Timestamp                           pollReturnTime_;
		std::unique_ptr<Poller>             poller_;				//分离器
        std::unique_ptr<TimerQueue>         timerQueue_;			//事件队列
        int64_t                             iteration_;
		...
		// scratch variables
		ChannelList                         activeChannels_;		//channel链表
		Channel*                            currentActiveChannel_;		//当前激活的channel
