...
////弱指针的lock会在该指针过期前返回shared_ptr,
//若已经过期就会返回一个空的共享式智能指针。
//判断过期的函数。
//expired()函数。
//弱指针，是为了解决共享式智能指针循环引用所使用的一种指针，它不具有普通指针的行为，没有重载
//*操作符和->操作符，它的作用在于协助共享式智能指针工作。具有资源的观测权，但是没有共享资源，
//它的构造不会引起引用技术的增加，使用它可以观察资源的引用计数，lock()

//weak_ptr<typename> 拥有三个函数
//(1)use_count()
//(2)expired()
//(3)lock()
