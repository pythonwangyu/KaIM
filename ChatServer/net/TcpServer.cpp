...
//创建connection对象，将他挂载到对应的对象上。
void TcpServer::newConnection(int sockfd, const InetAddress& peerAddr)
{
    loop_->assertInLoopThread();
    EventLoop* ioLoop = eventLoopThreadPool_->getNextLoop();        //选择一个IOLoop(轮询的模式).
    char buf[32];
    snprintf(buf, sizeof buf, ":%s#%d", hostport_.c_str(), nextConnId_); //初始化的时候为1，在TcpServer中初始化
    ++nextConnId_;
